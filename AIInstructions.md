...
- `Color.value` is deprecated; use `color.toARGB32()` to get the 32-bit ARGB int (e.g. when persisting a seed/accent color to JSON/Appwrite) and `Color(intValue)` to read it back.
...
- ❌ **Cascades (`..`) — avoid them; resolve every cascade into explicit statements on a named variable.** The `..` operator returns the *receiver* rather than the method's result, which is unfamiliar and confusing for non-expert readers. Assign the object to a `final` variable once, then call each method on its own line (see the worked example below). Also avoid spread-heavy one-liners, complex **collection-if/for** inside big widget trees, and nested ternaries.
...
### Example — resolving a cascade (do this everywhere `..` would appear)

**❌ Cascade (`..` returns the object, not the method result — confusing for non-experts):**
```dart
...
Identical behavior; the second form reads top-to-bottom with no `..` to decode.
...
- **`lib/widgets/`** — reusable shared widgets, including consistent **loading / error / empty** state widgets used across all `AsyncValue` views. **Extract reused form controls into `lib/widgets/forms/`** (e.g. `AppTextField` for label+icon inputs and `AppPasswordField` for a password input with a built-in show/hide toggle) so field styling is defined once and every screen stays consistent. A reusable password field MUST own its own visibility state (a `useState` inside a `HookWidget`) instead of taking a shared toggle from the caller, so it can be dropped in any number of times.
- **`lib/theme/`** — centralized Material 3 theming: `AppTheme.build({brightness, seedColor})` (one place owns colors, font sizes and component themes) plus the accent-color preset palette (`accent_colors.dart`).
...
- Base keys: `appTitle`, `welcome`, `login`, `logout`, `email`, `password`, `settings`, `profile`, `about`, `help`, `theme`, `language`, `darkMode`, `accentColor`, `version`, `logs`, `developerMode`.
...
- **Material 3** (`useMaterial3: true`); build BOTH the light and dark `ThemeData` from a single seed via `ColorScheme.fromSeed(...)`. **Centralize theme construction in `lib/theme/app_theme.dart` (`AppTheme.build({brightness, seedColor})`) — NOT inline in `app.dart`.** Dark/light is driven by `ThemeService` + `UserSettings.isDarkMode`.
- **Font sizes and component styling belong in the `ThemeData` (e.g. `navigationRailTheme`, `textTheme`), defined once in `AppTheme` — never hard-coded at each call site.** To restyle e.g. the sidebar labels, edit the theme, not every widget.
- **Accent color as a per-user setting (best practice = one seed):** store the seed color in `UserSettings` as an ARGB int (default the brand seed) and let the user pick it on the Settings page from a **curated preset palette** (`lib/theme/accent_colors.dart`); `app.dart` watches it and re-themes the whole app live. Prefer a fixed preset palette over a free color-wheel picker — no extra dependency, and it stays testable.
...
- ❌ Cascade operators (`..`) anywhere in `lib/` — resolve each into explicit statements on a named `final` variable (see the worked example in "Code Style & Target Audience").
- ❌ Hard-coding font sizes / component styling at the call site instead of in `AppTheme`'s `ThemeData`; building `ThemeData` inline in `app.dart` instead of in `lib/theme/`.
...
## Hooks & reactive gotchas (MUST know)

- **Widget state set in an event handler is lost across a provider-driven remount.** If an action (e.g. a switch's `onChanged`) both sets widget-local state (like pre-filling a `TextEditingController`) AND changes a provider the router guard watches, the guard can bounce the route (e.g. `login → splash → login`) and **remount the view**, re-initializing its hooks and discarding the value. Fix: derive that widget state from a `useEffect` keyed on the source value, so it re-applies on every (re)mount — do not set it once in the handler.
- Corollary: keep provider writes and pure UI-only state changes separate in your head — anything downstream of `currentUserProvider` (or another guard input) can trigger a remount.

---

## Optional: demo / offline mode (SHOULD — gated, no backend needed)

A login-page **"Demo mode"** switch that lets the whole app run on in-memory fake data with no Appwrite and no real login — useful for showcasing and for UI work offline. Implement it as a **provider swap, not a parallel code path**:

- `authServiceProvider` / `databaseServiceProvider` return fake implementations (`implements AuthService` / `DatabaseService`, living in `lib/services/demo/`, seeded with sample data) when a `demoModeProvider` is on. Because those providers are watched by `currentUserProvider`, flipping the flag re-evaluates auth and re-themes the app **with no restart**.
- **Security — two-layer gate (MUST):** a runtime-only toggle must NEVER be able to bypass auth in a shipped binary. Gate demo mode behind a **compile-time** flag `AppConfig.demoModeAllowed` (`bool.fromEnvironment`, via `--dart-define`) OR `kDebugMode`. Effective demo = `persistedToggle && (demoModeAllowed || kDebugMode)`. When the gate is off, the switch is hidden AND the fakes are unreachable, even if a stale preference says otherwise.
- Keep it faithful to the real flow: the demo account starts **logged out** so the switch pre-fills the normal login form (via the `useEffect` pattern above) and the user presses Login as usual; logout also turns demo mode off. Show an unmistakable **DEMO** badge in the shell while it is active.
- The demo fakes in `lib/services/demo/` mirror the hand-written test fakes in `test/fakes/` — same interfaces, different purpose (one serves seed data at runtime, the other records calls in tests).
...
- [ ] Theme is built in `lib/theme/AppTheme` (not inline in `app.dart`); font sizes / component styles live in the `ThemeData`, not at call sites.
- [ ] Accent (seed) color is a per-user `UserSettings` field, chosen from a preset palette on the Settings page, and re-themes the app live.
- [ ] Reused form controls are extracted to `lib/widgets/forms/` (`AppTextField`, self-contained `AppPasswordField`) and used by both Login and Settings.
- [ ] No cascade operators (`..`) in `lib/` — all resolved into explicit statements.
- [ ] (If demo mode is included) it is a provider swap gated by a compile-time flag; the switch is hidden and the fakes unreachable when the gate is off.